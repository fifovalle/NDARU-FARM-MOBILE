...
import useDetailSaranaPertanianPopuler from "../../hooks/useDetailSaranaPertanianPopuler";
import formatRupiah from "../../utils/formatRupiah";

export default function SayuranPopuler() {
  const { id } = useGlobalSearchParams();
  const pengarah = useRouter();
  const [kuantitasSaranaPertanianPopuler, setKuantitasSaranaPertanianPopuler] =
    useState(1);

  const { saranaPertanianPopuler, memuatSaranaPertanianPopuler } =
    useDetailSaranaPertanianPopuler(id);
        ...
        <ScrollView className="bg-[#E7E8E2] rounded-lg">
          <View className="bg-[#E7E8E2] mb-4 flex items-center justify-center">
            <View className="w-full h-80">
              <Image
                source={{ uri: saranaPertanianPopuler.Gambar }}
          ...
          <View className="rounded-t-[30px] bg-white">
            ...
            >
              {saranaPertanianPopuler.Nama}
            </Text>
            <Text
              style={{ fontFamily: gayaHuruf.lexend400 }}
              className="text-md ml-6 text-black"
            >
              {saranaPertanianPopuler.Jenis} Bulan
            </Text>
            <View className="mx-auto w-[90%]">
              ...
              >
                {saranaPertanianPopuler.Deskripsi}
              </Text>
            </View>
            ...
            <View className="flex-row items-center justify-between mx-7 mt-6 mb-4">
              <View className="flex-row items-center">
                <TouchableOpacity
                  ...
                  onPress={() =>
                    kurangiKuantitas(
                      kuantitasSaranaPertanianPopuler,
                      setKuantitasSaranaPertanianPopuler
                    )
                  }
                >
              ...
              >
                {formatRupiah(
                  saranaPertanianPopuler.Harga * kuantitasSaranaPertanianPopuler
                )}
              </Text>
            </View>
          </View>
